...
export class AddressRepository {
  ...
  async create(addressData: Partial<Address>): Promise<Address> {
    ...
    if (addressData.isDefault) {
      await this.unsetDefaultAddress(addressData.profileId);
    }
  ...
  async update(id: number, addressData: Partial<Address>): Promise<Address> {
    if (addressData.isDefault) {
      ...
      await this.unsetDefaultAddress(address.profileId);
    }
  ...
  private async unsetDefaultAddress(profileId: number): Promise<void> {
